SELECT * FROM smallable2_datawarehouse.b_stocks
...
sum(CASE WHEN bs.quantity_type_id = 1 THEN bs.price_ht ELSE 0 END) -
sum(CASE WHEN bs.quantity_type_id IN (4, 5, 6) THEN bs.price_ht ELSE 0 END) AS stock_vn
FROM
...
sum(CASE WHEN bs.quantity_type_id = 1 THEN bs.price_ht ELSE 0 END) -
sum(CASE WHEN bs.quantity_type_id IN (4, 5, 6) THEN bs.price_ht ELSE 0 END) AS stock_vn1
FROM
smallable2_datawarehouse.b_stocks bs
WHERE
...
sum(CASE WHEN bs.quantity_type_id = 1 THEN bs.price_ht ELSE 0 END) -
sum(CASE WHEN bs.quantity_type_id IN (4, 5, 6) THEN bs.price_ht ELSE 0 END) AS stock_vs1
FROM
smallable2_datawarehouse.b_stocks bs
WHERE
...
------------------ REQUÊTE ------------------
SELECT
v.*, s.stock_pe23, s.stock_pe23_s1, s.stock_pe22, s.stock_vn_pe23, s.stock_vs1_pe23_s1, s.stock_vn1_pe22, s.stock_residuel, s.stock_residuel_s1
FROM
...
sum(CASE WHEN bp.product_id IN (SELECT pid FROM saison WHERE s = 'PE24') THEN sn.stock_vn ELSE NULL END) AS stock_vn_pe23,
sum(CASE WHEN bp.product_id IN (SELECT pid FROM saison WHERE s = 'PE24') THEN ss1.stock_vs1 ELSE NULL END) AS stock_vs1_pe23_s1,
sum(CASE WHEN bp.product_id IN (SELECT pid FROM saison WHERE s = 'PE23') THEN sn1.stock_vn1 ELSE NULL END) AS stock_vn1_pe22,
