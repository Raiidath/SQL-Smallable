...
LEFT JOIN (
	...
	WHERE
		tr.ref_date_2 BETWEEN '2022-10-14' AND toDate(NOW()-1)
	GROUP BY
		sku_id) t ON
	t.sku_id = s.sku_id
LEFT JOIN (
	...
	FROM
		...
		FROM
			smallable2_datawarehouse.b_skus_x_days
		WHERE
			ref_date BETWEEN '2022-10-14' AND toDate(NOW()-1))
	GROUP BY
		product_id
	ORDER BY
...
LEFT JOIN (SELECT dp.id_product AS pid, dp.value AS discount FROM smallable2_descriptor.discount_product dp WHERE dp.id_discount = 43) s_ge ON s_ge.pid = p.product_id
LEFT JOIN (SELECT dp.id_product AS pid, dp.value AS discount FROM smallable2_descriptor.discount_product dp WHERE dp.id_discount = 44) s_eu ON s_eu.pid = p.product_id
WHERE
	p.outlet = 'NON'
	AND saison != 'AH23'
	...
	AND acheteur IN ('SERPIL','LAURA', 'BEATRICE', 'LOUISE', 'MARYSE', 'KATIA', 'TASSIANA', 'EMILIE' , 'HSIAO-ANNE', 'ALICE', 'AURELIE', 'LUCIE', 'MAYSE', 'VALENTINE', 'ELODIE', 'PAULINE', 'PAULINE M', 'LEA', 'MARINE G', 'ELENA B', 'MARLENE', 'OPHELIE', 'ELENA')
